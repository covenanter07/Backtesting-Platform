...
def monthly_fundamental():
    monthly = {}
    monthly["當月營收"] = data.get_adj('當月營收')
    monthly["上月營收"] = data.get_adj('上月營收')
    monthly["上月增減百分比"] = data.get_adj('上月比較增減(%)')
    monthly["去年當月營收"] = data.get_adj('去年當月營收')
    monthly["去年同月增減百分比"] = data.get_adj('去年同月增減(%)')
    monthly["去年累計營收"] = data.get_adj('去年累計營收')
    monthly["本益比"] = data.get_adj('本益比')
    #monthly["本益比"].fillna(method = 'ffill',axis=1 ,limit=None).reindex(every_month)
    monthly_fundamental = pd.DataFrame({name: df.unstack() for name, df in monthly.items()}) 
    monthly_fundamental.index = monthly_fundamental.index.set_names(['stock_id', 'date'])

    return monthly_fundamental#.transpose()
